...
function gui_framework:CreateToggle(parent, text, default, callback, group)
    ...
    toggle.MouseButton1Click:Connect(function()
        if group then
            for _, otherToggle in ipairs(parent:GetChildren()) do
                if otherToggle:IsA("TextButton") and otherToggle ~= toggle then
                    local otherCheck = otherToggle:FindFirstChild("Frame") and otherToggle.Frame:FindFirstChild("TextLabel")
                    if otherCheck then
                        otherCheck.Text = ""
                        otherToggle.BackgroundColor3 = Color3.fromRGB(50, 50, 50)
                    end
                end
            end
        end
...
function gui_framework:CreatePlayerDropdown(parent, callback)
    ...
    local function updatePlayerList()
        ...
        container:FindFirstChildOfClass("UIListLayout") or layout:Clone().Parent = container
        players = {}
        for _, p in ipairs(Players:GetPlayers()) do
            if p ~= LocalPlayer then
                table.insert(players, p.Name)
    ...
    Players.PlayerRemoving:Connect(function(player)
        if dropdown.Text == player.Name then
            dropdown.Text = "Select..."
        end
        updatePlayerList()
    end)
